...
func TestLoadFile(t *testing.T) {
	...
	saveMe := myStruct{}
	_, err := LoadConfig("test.json", &saveMe)
	if err == nil {
		t.Fatal(err)
	}
	...
	saveMe1 := myStruct{}
	_, err = LoadConfig("test.json", &saveMe1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSaveLoad(t *testing.T) {
	...
	newConfig, err := NewConfig(&loadMe)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDiff(t *testing.T) {
	...
	newConfig, err := NewConfig(&mismatch)
	if err != nil {
		t.Fatal(err)
	}
